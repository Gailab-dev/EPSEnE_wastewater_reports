...
### 1.2 시뮬레이션 모델 (Stage 09)

방류수 HRT 기반 장기 예측(t+13h) 모델. 7개 타겟 중 **6개가 성능 미달**.

| 타겟 | R² | MAPE | 판정 |
|------|:--:|:----:|:----:|
| 방류_TN_hrt | 0.6045 | 3.74% | R² 미달 |
| 방류_TP_hrt | 0.8089 | 4.75% | R² 미달 |
| 방류_BOD_hrt | 0.4769 | 7.14% | R² 미달 |
| 방류_SS_hrt | 0.9138 | 2.44% | **통과** |
| 방류_NH4_hrt | 0.7380 | 15.63% | R²+MAPE 미달 |
| 방류_NO3_hrt | 0.6795 | 3.81% | R² 미달 |
| 방류_COD_hrt | 0.7581 | 3.78% | R² 미달 |

> 장기 예측 특성상 정확도 저하 불가피. 라그 특징 범위 확대 또는 재귀적 예측 전략 검토 필요.
...
EPSEnE에서 제공한 프론트엔드(`frontend/a2o-system/`)와 기능명세서/과업지시서/databook 간의 구조적 불일치가 확인됨. 상세: [menu_mapping.md](./menu_mapping.md)
...
> **참고**: 위 9건의 API는 **과업지시서에 없으며**, 기능명세서 부록(FUN-000~005)에서만 정의되어 있음. 프론트엔드가 해당 기능을 탑재하고 있으므로, EPSEnE 측과 협의하여 추가 개발 범위에 포함할지 결정이 필요함.
...
> **핵심 요청**: 프론트엔드가 24개 API를 전제로 설계되어 있으나, 과업지시서 기반 개발 범위는 15개(MFR)임. 나머지 9개(부록 FUN) API는 과업지시서에 포함되지 않으므로, EPSEnE 측과 협의하여 추가 개발 여부 및 우선순위를 결정해야 할 것으로 판단됨.
